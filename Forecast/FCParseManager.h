...
- (void) getSearchResultsForTerm:(NSString *)searchTerm includeProjects:(BOOL)searchProjects andArtists:(BOOL)searchArtists favoritesOnly:(BOOL)favoritesOnly inCategoriesWithCategoryIDs:(NSArray *)categoryIDs returnLocations:(BOOL)shouldReturnLocations inBackgroundWithBlock:(PFArrayResultBlock)block; // Gets all artists and/or projects that match the given search criteria
